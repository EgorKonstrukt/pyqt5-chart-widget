...
class _FunctionItem:
    ...
    def __init__(self, chart: "ChartWidget", fn: Callable[[List[float]], List[float]],
                 pen: QPen, label: str = "", resolution: float = 1.5):
        self._chart = chart
        self.fn = fn
        self.pen = pen
        self.label = label
    ...
    def evaluate(self, x_lo: float, x_hi: float, pixel_width: int) -> Tuple[List[float], List[Optional[float]]]:
        """
        Sample the function over [x_lo, x_hi] using pixel_width * resolution
        evenly-spaced points.  Returns (xs, ys).

        Results are cached by key (x_lo, x_hi, n_pts) so repeated calls for
        the same viewport incur no computation.
        """
        n_pts = max(4, int(pixel_width * self.resolution))
        key = (round(x_lo, 12), round(x_hi, 12), n_pts)
        if key == self._cache_key:
            return self._cached_xs, self._cached_ys
        from .math_utils import linspace
        xs = linspace(x_lo, x_hi, n_pts)
        try:
            raw = self.fn(xs)
        except Exception:
            raw = [None] * len(xs)
        ...
        for v in raw:
            if v is None:
                ...
                try:
                    fv = float(v)
                    ys.append(None if not math.isfinite(fv) else fv)
